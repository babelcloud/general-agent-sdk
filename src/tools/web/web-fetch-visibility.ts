...
type SanitizedElement = {
	...
	parentNode?: { removeChild(node: SanitizedElement): void } | null;
};

type SanitizedDocument = {
	querySelectorAll(selector: string): Iterable<SanitizedElement>;
	toString(): string;
};
...
function shouldRemoveElement(element: SanitizedElement): boolean {
...
export async function sanitizeHtml(html: string): Promise<string> {
	...
	let document: SanitizedDocument;
	try {
		const { parseHTML } = await import("linkedom");
		({ document } = parseHTML(sanitized) as { document: SanitizedDocument });
	} catch {
		return sanitized;
	}
