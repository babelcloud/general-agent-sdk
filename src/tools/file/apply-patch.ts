...
export function createApplyPatchTool(cwd: string): GeneralAgentTool {
  return {
    name: "apply_patch",
    description:
    ...
    async execute(_callId, params, signal) {
      const parsed = applyPatchSchema.parse(params);
      ...
      const result = await applyPatch(parsed.input, { cwd, signal });
...
export async function applyPatch(
  ...
  for (const hunk of parsed.hunks) {
    ...
    if (hunk.movePath) {
      ...
      await fs.mkdir(path.dirname(moveTarget.resolved), { recursive: true });
...
export async function resolveApplyPatchTargets(
  input: string,
  cwd: string,
): Promise<ApplyPatchTarget[]> {
  const parsed = parsePatchText(input);
  const targets: ApplyPatchTarget[] = [];
  const seen = new Set<string>();

  for (const hunk of parsed.hunks) {
    const target = await resolvePatchPath(hunk.path, cwd);
    if (!seen.has(target.resolved)) {
      seen.add(target.resolved);
      targets.push(target);
    }

    if (hunk.kind === "update" && hunk.movePath) {
      const moveTarget = await resolvePatchPath(hunk.movePath, cwd);
      if (!seen.has(moveTarget.resolved)) {
        seen.add(moveTarget.resolved);
...
async function resolvePatchPath(
  ...
  await assertWithinWorkspace(resolved, cwd);
  return {
    resolved,
...
async function assertWithinWorkspace(targetPath: string, cwd: string): Promise<void> {
  const lexicalWorkspaceRoot = path.resolve(cwd);
  const workspaceRoot = await fs.realpath(cwd).catch(() => lexicalWorkspaceRoot);
  const resolvedTarget = path.resolve(targetPath);
  const relativeTarget = path.relative(lexicalWorkspaceRoot, resolvedTarget);
  ...
  const existingParent = await findNearestExistingParent(resolvedTarget);
  if (!existingParent) {
    return;
  }

  const realParent = await fs.realpath(existingParent);
  const relativeParent = path.relative(workspaceRoot, realParent);
  if (
    relativeParent === ".." ||
...
async function findNearestExistingParent(targetPath: string): Promise<string | null> {
