...
function resolveBraveMode(searchConfig?: WebSearchConfig): "web" | "llm-context" {
	return searchConfig?.brave?.mode === "llm-context" ? "llm-context" : "web";
}
...
function resolveBraveApiKey(
	...
	return (
		readConfiguredSecretString(searchConfig?.apiKey) ??
		readConfiguredSecretString(searchConfig?.brave?.apiKey) ??
		readProviderEnvValue(["BRAVE_API_KEY", "BRAVE_SEARCH_API_KEY"], env)
	);
}
...
async function runBraveLlmContextSearch(params: {
	...
	return await withTrustedWebSearchEndpoint(
		...
		async (res) => {
			if (!res.ok) {
				await throwWebSearchApiError(res, "Brave LLM Context");
			}

			const data = (await readJsonResponse(res)) as unknown as BraveLlmContextResponse;
...
async function runBraveWebSearch(params: {
	...
	return await withTrustedWebSearchEndpoint(
		...
		async (res) => {
			if (!res.ok) {
				await throwWebSearchApiError(res, "Brave Search");
			}

			const data = (await readJsonResponse(res)) as unknown as BraveSearchResponse;
			...
			return results.map((entry) => {
				...
				const url = entry.url ?? "";
				return {
					title: title ? wrapSearchContent(title) : "",
					url,
					description: description ? wrapSearchContent(description) : "",
	...
	return z
		.object({
			...
			freshness: z
				.string()
				.optional()
				.describe("Filter by time: 'day' (24h), 'week', 'month', or 'year'."),
			date_after: z
				.string()
				.optional()
				.describe("Only results published after this date (YYYY-MM-DD)."),
			date_before: z
				.string()
				.optional()
				.describe("Only results published before this date (YYYY-MM-DD)."),
			search_lang: z
				.string()
				.optional()
				.describe(
...
function missingBraveKeyPayload() {
	return {
		...
		message:
			"web_search (brave) needs a Brave Search API key. Configure tools.web.search.apiKey or set BRAVE_API_KEY / BRAVE_SEARCH_API_KEY.",
		docs: "https://docs.openclaw.ai/tools/web",
	};
}

export function createBraveWebSearchProvider(): WebSearchProviderEntry {
	return {
		id: "brave",
		label: "Brave Search",
		...
		getCredentialValue: (searchConfig) =>
			readConfiguredSecretString(searchConfig?.apiKey) ??
			readConfiguredSecretString(searchConfig?.brave?.apiKey),
		createTool: ({ searchConfig, env }) => {
			const braveMode = resolveBraveMode(searchConfig);

			return {
				description:
					braveMode === "llm-context"
						? "Search the web using Brave Search LLM Context API. Returns pre-extracted page content optimized for LLM grounding."
						: "Search the web using Brave Search API. Supports region-specific and localized search via country and language parameters. Returns titles, URLs, and snippets for fast research.",
				parameters: createBraveSchema(),
				execute: async (args) => {
					const apiKey = resolveBraveApiKey(searchConfig, env);
					if (!apiKey) {
					...
					if (rawFreshness && (rawDateAfter || rawDateBefore)) {
						return {
							...
							message:
								"freshness and date_after/date_before cannot be used together. Use either freshness or a date range, not both.",
							docs: "https://docs.openclaw.ai/tools/web",
						};
					}
					...
					if (braveMode === "llm-context") {
						...
						const payload = {
							...
							results: results.map((entry) => ({
								title: entry.title ? wrapSearchContent(entry.title) : "",
								url: entry.url,
								snippets: entry.snippets.map((snippet) => wrapSearchContent(snippet)),
